...
function App() {
  ...
  useEffect(() => {
    if (
      selectedEntry &&
      isToday(selectedEntry.timestamp) &&
      selectedEntry.paragraphs
    ) {
    ...
    const loadData = async () => {
      try {
        ...
        if (entries.length > 0) {
          ...
          try {
            const paragraphs = await journalAPI.getEntryParagraphs(
              entryToLoad.entry_id
            );
            setSelectedEntry({
              ...entryToLoad,
  ...
  const handleEntryDelete = async (entry: JournalEntry) => {
    let userConfirmed = window.confirm(
      `Are you sure you want to delete the entry from ${new Date(
        entry.timestamp
      ).toLocaleDateString()}? This will delete all paragraphs and cannot be undone.`
    );
        ...
        if (selectedEntry?.entry_id === entry.entry_id) {
          if (updatedEntries.length > 0) {
            ...
            try {
              ...
              console.error(
                "Failed to load paragraphs for new selected entry:",
                error
              );
  ...
  const handleParagraphDelete = async (paragraph: ChatParagraph) => {
    ...
    if (userConfirmed) {
      try {
        ...
        // Remove from local state
        if (selectedEntry) {
          const updatedParagraphs = selectedEntry.paragraphs.filter(
